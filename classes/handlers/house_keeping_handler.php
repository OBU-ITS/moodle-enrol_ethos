...
class house_keeping_handler {
    ...
    public function handlePersonHoldsCleaning() {
        ...
        foreach($users as $user) {
            $user->getCustomData()->personHolds = (string) $this->personHoldsService->cleanHoldsProfileField($user->getCustomData()->personHolds);
    ...
    public function handleStudentAdviserRelationships()
    {
        ...
        $updatedUsers = new obu_users_info();
        foreach ($users as $user) {
            $user->getCustomData()->studentAdviser = (string) $this->studentAdvisorRelationshipService->cleanStudentAdvisorProfileField($user);
            $updatedUsers->addUser($user);
        }
